...
const PageHeader = ({ projectName, rightText, now }: { projectName: string; rightText: string; now: string }) => (
  <View style={s.header}>
    <Text style={s.headerTitle}>HVHZ Calc Pro · FastenerCalc HVHZ — {projectName}</Text>
...
const FastenerCalcPdfReport = ({
  ...
  return (
    <Document title={`FastenerCalc HVHZ — ${projectName}`} author="HVHZ Calc Pro">

      {/* ══════════════ PAGE 1: COVER ══════════════ */}
      <Page size="LETTER" style={s.coverPage}>
        <View style={s.coverContent}>
          <Text style={{ fontSize: 10, color: c.gray, marginBottom: 8 }}>HVHZ Calc Pro</Text>
