...
const FastenerPdfExportButton = () => {
  ...
  const canDownloadClean = isProSubscriber || hasReportCredit || unlocked;
  ...
  const handleDownload = async (watermark: boolean) => {
    ...
    try {
      const blob = await pdf(
        <FastenerCalcPdfReport inputs={inputs} outputs={outputs} tas105Outputs={tas105Outputs} projectName={projectName || 'Untitled Project'} preparedBy={preparedBy} jobAddress={jobAddress} watermark={watermark} engineer={profile} />
      ).toBlob();
      const url = URL.createObjectURL(blob);
      const a = document.createElement('a');
      a.href = url;
      a.download = `FastenerCalc${watermark ? '_SAMPLE' : ''}_${(projectName || 'Report').replace(/\s+/g, '_')}.pdf`;
      a.click();
  ...
  const generateUnsignedPdf = async (): Promise<Blob> => {
    ...
    return pdf(
      <FastenerCalcPdfReport inputs={inputs} outputs={outputs} tas105Outputs={tas105Outputs} projectName={projectName || 'Untitled Project'} preparedBy={preparedBy} jobAddress={jobAddress} watermark={false} engineer={profile} />
    ).toBlob();
  };
  ...
  const handlePurchase = async () => {
    ...
    setPurchasing(true);
    try {
      const { data, error } = await supabase.functions.invoke('create-report-payment', { body: { reportType: 'fastener' } });
    ...
    <>
      <Dialog open={open} onOpenChange={setOpen}>
        ...
        <DialogContent className="sm:max-w-md">
          <DialogHeader><DialogTitle className="font-display">Export Fastener Report</DialogTitle></DialogHeader>
            ...
            {canDownloadClean ? (
              ...
              <>
                <div className="rounded-lg border border-border bg-card p-4 space-y-3">
                  ...
                  <p className="text-[10px] text-muted-foreground">Branded with your firm info. Full derivation chain, fastener patterns, insulation, TAS 105, and signature block.</p>
