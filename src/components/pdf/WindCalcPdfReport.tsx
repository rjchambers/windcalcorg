...
const WindCalcPdfReport = ({ inputs, outputs, projectName = 'Untitled Project', preparedBy = '', watermark = false }: Props) => {
  const now = new Date().toLocaleDateString('en-US', { year: 'numeric', month: 'long', day: 'numeric' });

  return (
    <Document title={`HVHZ Calc Pro — ${projectName}`} author="HVHZ Calc Pro">
      {/* Cover Page */}
      <Page size="LETTER" style={s.coverPage}>
        <View style={s.coverContent}>
          <Text style={s.coverTitle}>HVHZ Calc Pro</Text>
        ...
        <View style={{ ...s.footer, bottom: 30 }}>
          <Text style={{ color: colors.gray, fontSize: 7 }}>Generated by HVHZ Calc Pro — For review by licensed PE</Text>
        </View>
      ...
      <Page size="LETTER" style={s.page}>
        <View style={s.header}>
          <Text style={s.headerTitle}>HVHZ Calc Pro — {projectName}</Text>
